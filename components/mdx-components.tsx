...
export const MDXComponents = {
  ...
  a: ({ className, ...props }: React.HTMLAttributes<HTMLAnchorElement>) => (
    <Link className={`font-medium underline underline-offset-4 ${className}`} {...props} href={props.href || "#"} />
  ),
  ...
  pre: ({ className, ...props }: React.HTMLAttributes<HTMLPreElement>) => (
    <pre className={`mb-4 mt-6 overflow-x-auto rounded-lg border bg-black py-4 ${className}`} {...props} />
  ),
  code: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <code className={`relative rounded border px-[0.3rem] py-[0.2rem] font-mono text-sm ${className}`} {...props} />
  ),
